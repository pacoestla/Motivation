...
if (global.PriorityBar > priority && !dead) {
	...
	if (hspeed != 0) {
		...
		global.Damage += ((global.player_attack - global.Monster[0].Defense) + random(2));
		
		if (BarDistance <= 12) {
			global.Damage *= 1.4;
			
			if (global.BarCount > 1) audio_play_sound(sSelect, 20, false);
		}
		else {
		...
		global.Damage -= (global.Monster[0].Defense / 2);
		...
		if (global.player_attack - (global.Monster[0].Defense / 2) <= 0) {
			global.Damage = 0;
		}
	}
	hspeed = 0;
